...
/* 用户 table_name = user */
type User struct {
	Id            int           `json:"user_id"`                       //用户编号 默认自增
	Name          string        `orm:"size(32);unique"  json:"name"`   //用户昵称  unique表示唯一的
...
func init(){
	...
	orm.RegisterDataBase("default", "mysql", "demo:db123321@tcp(127.0.0.1:3306)/homedb?charset=utf8", 30)
